...
import { loadCrowniclesHelpData } from '../data/events.js';
...
export const specialEventsPage: HelpPage = {
  ...
  render(state: HelpState, context: HelpRenderContext) {
    ...
    const container = createHelpPageContainer(SPECIAL_ICON, t.name, t.intro);
    ...
    const specials = state.data.specialEvents.slice(0, PICKER_PAGE_SIZE);
